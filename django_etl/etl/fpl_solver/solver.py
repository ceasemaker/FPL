# Upstream: https://github.com/lisovoy7/fpl-solver (commit 5b62c4e)
# This file is copied from the fpl-solver project with minimal modifications.
...
# Callers spell positions differently (the FPL API and this project's API views
# use "GKP"); normalise to the keys SQUAD_COMPOSITION is defined on, or the
# goalkeeper constraint silently matches no players and the model is infeasible.
POSITION_ALIASES = {
    'GKP': 'GK',
    'GOALKEEPER': 'GK',
    'G': 'GK',
...
def normalize_position(position: object) -> object:
    """Map a caller's position label onto the solver's vocabulary."""
...
class FPLSolver:
    ...
    def __init__(
        ...
        risk_strength: Optional[float] = None,
    ):
        """
        ...
        Args:
            ...
            risk_profile: Rank objective: 'protect', 'balanced', or 'chase'.
            risk_strength: Optional mean-variance coefficient override.
        """
        ...
        self.risk_profile = risk_profile if risk_profile in {'protect', 'balanced', 'chase'} else 'balanced'
        default_risk_strength = {'protect': -0.04, 'balanced': 0.0, 'chase': 0.025}
        self.risk_strength = (
            float(risk_strength)
            if risk_strength is not None
            else default_risk_strength[self.risk_profile]
        )
        ...
        self.initial_bank = 0.0
        self.initial_selling_prices = {}
        self.prob = None
        self.variables = {}
    ...
    def load_predictions(self, predictions_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'position' in predictions.columns:
            predictions['position'] = predictions['position'].map(normalize_position)
        ...
        if 'predicted_variance' in predictions.columns:
            agg_dict['predicted_variance'] = 'sum'
        if 'effective_ownership' in predictions.columns:
            agg_dict['effective_ownership'] = 'first'
    ...
    def load_player_data(
        ...
        self.players['position'] = self.players['position'].map(normalize_position)
    ...
    def set_initial_squad(
        ...
        Args:
            ...
            bank: Cash in bank in the same price units as player values.
            selling_prices: Personalized sale price for each initially-owned player.
        """
        ...
        self.initial_bank = max(0.0, float(bank))
        self.initial_selling_prices = selling_prices or {}
    ...
    def create_objective(self) -> None:
        ...
        for _, row in self.predictions.iterrows():
            ...
            if key in self.expected_points:
                ...
                self.expected_points[key] = row['predicted_points']
            self.predicted_variance[key] = max(0.0, float(row.get('predicted_variance', 0.0) or 0.0))
            self.effective_ownership[key] = max(0.0, float(row.get('effective_ownership', 0.0) or 0.0))
        ...
        for t in gameweeks:
            ...
            for p in players:
                ...
                # Relative-rank variance versus the ownership benchmark. The
                # non-owner term is constant and can be omitted. For a starter,
                # variance changes by (1-e)^2-e^2; captaincy then changes it to
                # (2-e)^2. A negative coefficient protects rank; a positive one
                # deliberately seeks variance when chasing.
                if self.risk_strength:
                    variance = self.predicted_variance.get((p, actual_gw), 0.0)
                    eo = self.effective_ownership.get((p, actual_gw), 0.0)
                    starter_variance_delta = (1.0 - eo) ** 2 - eo ** 2
                    captain_variance_delta = (2.0 - eo) ** 2 - (1.0 - eo) ** 2
                    objective_terms.append(
                        self.risk_strength
                        * variance
                        * starter_variance_delta
                        * self.variables['y'][(p, t)]
                    )
                    objective_terms.append(
                        self.risk_strength
                        * variance
                        * captain_variance_delta
                        * self.variables['c'][(p, t)]
                    )
    ...
    def add_squad_flow_constraints(self) -> None:
        ...
        for p in players:
            if self.initial_squad:
                ...
                # Open-pool mode constructs the first squad directly. It is
                # not a 15-transfer wildcard and must carry no transfer cost.
                self.prob += (self.variables['s'][(p, 1)] == 0, f"Open_Pool_No_In_{p}")
                self.prob += (self.variables['r'][(p, 1)] == 0, f"Open_Pool_No_Out_{p}")
    ...
    def add_cash_flow_constraints(self) -> None:
        """Track personalized cash using actual sale prices for the current squad."""
        if not self.initial_squad:
            return

        players = self.players['element'].tolist()
        market_price = dict(zip(self.players['element'], self.players['value']))
        sale_price = {
            player_id: float(self.initial_selling_prices.get(player_id, market_price[player_id]))
            for player_id in players
        }
        for t in range(1, self.T + 1):
            transfer_income = pulp.lpSum(
                sale_price[p] * self.variables['r'][(p, t)] for p in players
            )
            transfer_spend = pulp.lpSum(
                market_price[p] * self.variables['s'][(p, t)] for p in players
            )
            if t == 1:
                self.prob += (
                    self.variables['bank'][t]
                    == self.initial_bank + transfer_income - transfer_spend,
                    "Initial_Bank_Flow",
                )
            else:
                self.prob += (
                    self.variables['bank'][t]
                    == self.variables['bank'][t - 1] + transfer_income - transfer_spend,
                    f"Bank_Flow_{t}",
                )
    ...
    def add_squad_composition_constraints(self) -> None:
        ...
        for t in gameweeks:
            ...
            if t not in fh_internal_gws and not self.initial_squad:
                self.prob += (
    ...
    def build_model(self) -> None:
        ...
        self.add_cash_flow_constraints()
    ...
    def extract_solution(self) -> Dict:
        ...
        for t in gameweeks:
            ...
            solution['bank'][t] = float(self.variables['bank'][t].varValue or 0.0)
