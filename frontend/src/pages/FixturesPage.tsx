...
interface FixtureMatrix { current_gameweek: number; start_gameweek: number; end_gameweek: number; horizon: number; market: { available: boolean; snapshot_date: string | null; source: string | null; priced_fixture_count: number; total_fixture_count: number; type: string; includes_clean_sheet_odds: boolean; note: string }; teams: TeamRow[] }
...
export function FixturesPage() {
  ...
  return <main className="aero-page fixtures-page">
    ...
    <p className="aero-note fixtures-context">{basis === "market" ? `${data?.market.type ?? "1X2 match-result odds"} · ${data?.market.priced_fixture_count ?? 0}/${data?.market.total_fixture_count ?? 0} fixtures priced · ${data?.market.source ?? "source unavailable"} · updated ${data?.market.snapshot_date ?? "unavailable"}. Higher win probability means an easier fixture. “${winAllLabel}” and “Win 1+” assume fixture results are independent. These are not clean-sheet odds.` : basis === "blend" ? `Combined order uses percentile ranks: 70% current average win probability and 30% official average FDR. Market coverage is ${data?.market.priced_fixture_count ?? 0}/${data?.market.total_fixture_count ?? 0} fixtures; missing prices fall back to FDR. “${winAllLabel}” and “Win 1+” assume fixture results are independent.` : "FDR is the official 1–5 fixture difficulty rating. Venue is shown as H or A. Lower average FDR means the easier five-match run."}</p>
